...
# Classifier prompt with calculation and datetime categories
...
class BasicToolsChat(ChatInterface):
    ...
    def _classify_query(self, state: ToolState) -> ToolState:
    ...
    def _route_query(self, state: ToolState) -> Literal["factual", "analytical", "comparison", "definition", "calculation", "datetime", "default"]:
        """Route to the appropriate node based on category."""
        return state["category"]
    
    def _factual_response(self, state: ToolState) -> ToolState:
        """Generate factual response."""
        chain = RESPONSE_PROMPTS["factual"] | self.llm | StrOutputParser()
        response = chain.invoke({"question": state["question"]})
        return {**state, "response": response}
    
    def _analytical_response(self, state: ToolState) -> ToolState:
        """Generate analytical response."""
        chain = RESPONSE_PROMPTS["analytical"] | self.llm | StrOutputParser()
        response = chain.invoke({"question": state["question"]})
        return {**state, "response": response}
    
    def _comparison_response(self, state: ToolState) -> ToolState:
        """Generate comparison response."""
        chain = RESPONSE_PROMPTS["comparison"] | self.llm | StrOutputParser()
        response = chain.invoke({"question": state["question"]})
        return {**state, "response": response}
    
    def _definition_response(self, state: ToolState) -> ToolState:
        """Generate definition response."""
        chain = RESPONSE_PROMPTS["definition"] | self.llm | StrOutputParser()
        response = chain.invoke({"question": state["question"]})
        return {**state, "response": response}
    
    def _default_response(self, state: ToolState) -> ToolState:
        """Generate default response."""
        chain = RESPONSE_PROMPTS["default"] | self.llm | StrOutputParser()
        response = chain.invoke({"question": state["question"]})
        return {**state, "response": response}
    
    def _calculation_response(self, state: ToolState) -> ToolState:
    ...
    def _datetime_response(self, state: ToolState) -> ToolState:
